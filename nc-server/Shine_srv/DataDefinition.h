...
//LINES BELOW ADDED FABIO

typedef struct THREAD_ARGS{
	cf_data coloring_output;
	int num_colori;
    int* coloring_p;
//    data_matrix coloring_data;
}thread_args;

typedef struct THREAD_TCP_ARGS{
    int thread_connFd;
    int thread_noThread;
}thread_tcp_args;

/*
typedef struct THREAD_TRASM{
	int i;
	cf_data* outputForColoringP;
	vector<string>* filesP;
	vector<vector<char>>* coded_dataP;
	header_transmission* headerP;
}thread_trasm;
*/
